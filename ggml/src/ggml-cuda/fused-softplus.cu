...
static __global__ void fused_softplus_kernel(
    const float * x, const float * add_a, const float * add_b, float * dst,
    ...
    const uint2 dm = fast_div_modulo((uint32_t)tid, ne0);
...
void ggml_cuda_op_fused_softplus(ggml_backend_cuda_context & ctx, ggml_tensor * dst, ggml_tensor * add_src0, ggml_tensor * add_src1) {
    ...
    const int64_t num_blocks = (ne + CUDA_FUSED_SOFTPLUS_BLOCK_SIZE - 1) / CUDA_FUSED_SOFTPLUS_BLOCK_SIZE;
