...
static bool ggml_cuda_graph_update_required(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        // check src tensor properties that can change independently of the node itself
        // (e.g. when the backend scheduler creates new copy tensors, or KV cache views change shape)
